...
/**
 * LoginForm component with email/password and OAuth authentication
...
export function LoginForm({ onSwitchToSignUp, onSuccess, className }: LoginFormProps) {
  ...
  const { signIn, signInWithOAuth, resetPassword, error, clearError } = useAuthContext();
  ...
  const handlePasswordReset = async (e: React.FormEvent) => {
    ...
    if (!resetEmail.trim()) {
      setFormErrors(prev => ({ ...prev, general: 'Please enter your email address' }));
      return;
    }

    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(resetEmail)) {
      setFormErrors(prev => ({ ...prev, general: 'Please enter a valid email address' }));
      return;
    }
  ...
  return (
    <div className={cn('w-full max-w-md', className)}>
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow-soft p-6">
        ...
        <p className="text-gray-600 dark:text-gray-400 mb-6">
          Sign in to your VocabLens account
        </p>
